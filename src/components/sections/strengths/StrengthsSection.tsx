      ...
      >
        <div className="p-6 cursor-pointer" onClick={onToggle}>
          <div className="flex items-center justify-between">
            <h3
              className={`text-xl font-medium ${isOpen ? "text-black" : "text-[#6D6D6D]"} select-none`}
            >
...
const StrengthsSection: React.FC = () => {
  ...
  const [openStates, setOpenStates] = useState<boolean[]>(
    new Array(strengthsList.length).fill(false),
  );
  ...
  return (
    <section
      id="strengths"
      className="relative py-16 overflow-hidden mt-4 mb-24"
    >
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3/4 h-1/2 bg-gradient-to-b from-primary-500/20 to-transparent blur-3xl -z-10" />
